...
請務必遵守以下輸出格式要求，並嚴格分為五個主要章節輸出（不可漏失任何一章）：

1. **會議主題與時間**：擷取會議的主題與時間。
2. **與會者**：列出參與會議的人員。
3. **會議重點總結**：用 3 到 5 個重點總結會議內容。
4. **Action Items (待辦事項)**：明確列出接下來的待辦事項與負責人。
5. **${displayLang}翻譯版**：將上述第 1 點至第 4 點的所有內容，完整且逐字翻譯成高品質的專業${displayLang}。

請以 Markdown 格式輸出。除了第 5 點的${displayLang}翻譯版必須使用對應的${displayLang}書寫外，其他繁體中文部分必須使用中華民國台灣常用的**繁體中文**回覆，不要包含任何額外的問候語或結語。`;
