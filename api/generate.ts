...
請務必遵守以下輸出格式要求，並嚴格分為五個章節，不可遺漏任何章節：

1. **會議主題與時間**：（請以繁體中文撰寫）擷取會議的主題與時間。
2. **與會者**：（請以繁體中文撰寫）列出參與會議的人員。
3. **會議重點總結**：（請以繁體中文撰寫）用 3 到 5 個重點總結會議內容。
4. **Action Items (待辦事項)**：（請以繁體中文撰寫）明確列出接下來的待辦事項與負責人。
5. **${displayLang}翻譯版**：請將上述第 1 點至第 4 點的繁體中文內容，**完整且百分之百翻譯成專業的${displayLang}**（不可使用中文，必須完全使用${displayLang}書寫）。

【重要語言與格式限制】：
- 第 1 點至第 4 點必須使用台灣習慣的**繁體中文**回覆（例如：使用「待辦事項」而非「待辦事宜」）。
- 第 5 點必須**完全且僅使用${displayLang}**進行撰寫。
- 請以標準 Markdown 格式輸出，直接開始輸出報告內容，不要包含任何額外的自我介紹、問候語或結語。`;
